from ads.utils import NUMBERS, load_data_from_collection
import unittest
from random import randint
from ads.searching.linear_probing_ht import *


class TestLinearProbingHashST(unittest.TestCase):
    def setUp(self):

        self.zip_code = {
            'Lower East Side': 10003,
            'Lower Manhattan': 10004,
            10003: 'Lower East Side',
            10004: 'Lower Manhattan'
        }

        self.T1 = LinearProbingHashST()
        load_data_from_collection(NUMBERS, self.T1)

        self.T2 = LinearProbingHashST()
        self.assertTrue(self.T2.is_empty())

    def tearDown(self):
        for key in self.T1.keys_set():
            self.T1.delete(key)

        assert self.T1.is_empty()
        self.T1 = None

    def test_size(self):
        self.assertEqual(self.T1.size(), len(NUMBERS))

        self.assertEqual(self.T2.size(), 0)
        load_data_from_collection(self.zip_code, self.T2)
        self.assertEqual(self.T2.size(), len(self.T2.keys_set()))
        self.assertNotEqual(self.T2.size(), 0)

    def test_is_empty(self):
        self.assertFalse(self.T1.is_empty())

        self.assertTrue(self.T2.is_empty())
        for i in self.zip_code.keys():
            self.T2.put(i, self.zip_code[i])
        self.assertFalse(self.T2.is_empty())

    def test_get(self):
        self.assertEqual(self.T1.get('ZERO'), 0)
        self.assertEqual(self.T1.get('FIFTY'), 50)

        self.assertIsNone(self.T2.get('Lower East Side'))
        self.assertIsNone(self.T2.get(10003))
        for i in self.zip_code.keys():
            self.T2.put(i, self.zip_code[i])

        self.assertEqual(self.T2.get('Lower East Side'), 10003)
        self.assertEqual(self.T2.get(10003), 'Lower East Side')
        self.assertEqual(self.T2.get('Lower Manhattan'), 10004)
        self.assertEqual(self.T2.get(10004), 'Lower Manhattan')

        with self.assertRaises(ValueError):
            self.T2.get(None)
            self.T1.get(None)

    def test_contains(self):
        self.assertIn('THIRTY', self.T1)
        self.assertIn('ZERO', self.T1)
        self.assertNotIn('ZZR0', self.T1)

        self.assertNotIn('Lower East Side', self.T2)
        self.assertNotIn(10003, self.T2)
        for i in self.zip_code.keys():
            self.T2.put(i, self.zip_code[i])

        self.assertIn('Lower East Side', self.T2)
        self.assertIn(10003, self.T2)
        self.assertIn('Lower Manhattan', self.T2)
        self.assertIn(10004, self.T2)

        with self.assertRaises(ValueError):
            self.T2.contains(None)
            self.T1.contains(None)

    def test__resize(self):
        for i in 'ABCDEFGH':
            self.i = LinearProbingHashST()
            for i in self.zip_code.keys():
                self.i.put(i, self.zip_code[i])
            self.assertEqual(self.i.m, self.i.INIT_CAPACITY)
            self.assertEqual(self.i.n, len(self.zip_code))

        for i in 'ABCDEFGH':
            self.i._resize(ord(i))
            self.assertEqual(self.i.m, ord(i))
            self.assertEqual(self.i.n, len(self.zip_code))

    def test_put(self):
        for i in self.zip_code.keys():
            self.T2.put(i, self.zip_code[i])
            self.assertIn(i, self.T2)

        for i in range(100):
            self.T2.put(i, randint(0, 1000))
            self.assertIn(i, self.T2)

        with self.assertRaises(ValueError):
            self.T2.put(None, randint(0, 10))

        self.T2.put(10003, None)
        self.T2.put('Lower Manhattan', None)

        self.assertNotIn(10003, self.T2)
        self.assertNotIn('Lower Manhattan', self.T2)

    def test_delete(self):
        N = 1000
        for i in range(N):
            self.T2.put(i, randint(0, 1000))
            self.assertIn(i, self.T2)

        for i in range(N):
            self.T2.delete(i)
            self.T2.delete(randint(0, i))
            self.assertNotIn(randint(0, i), self.T2)

        self.assertTrue(self.T2.is_empty())

        with self.assertRaises(ValueError):
            self.T2.delete(None)
            self.T1.delete(None)

        for key in NUMBERS.keys():
            self.T1.delete(key)
            self.assertNotIn(key, self.T1)

    def test_keys_set(self):
        self.assertIn('ZERO', self.T1.keys_set())
        self.assertIn('FORTY', self.T1.keys_set())
        self.assertEqual(len(self.T1.keys_set()), len(NUMBERS))

        for i in self.zip_code.keys():
            self.T2.put(i, self.zip_code[i])
        self.assertIn(10003, self.T2.keys_set())
        self.assertIn(10004, self.T2.keys_set())
        self.assertEqual(len(self.T2.keys_set()), len(self.zip_code.keys()))
