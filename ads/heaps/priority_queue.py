# Taken from: MIT-6.006_Introduction_to_Algorithms
...
class PriorityQueue(Generic[T]):
    ...
    def __init__(self):
        ...
        self.heap: List = [None]  # To make the index 1-based.
        self.key_index: Dict[T, int] = {}  # key to index mapping.
    ...
    def _min_heapify(self, i):
        ...
        r = 2 * i + 1
        smallest = i
        if l <= len(self) and self[l] < self[i]: smallest = l
        if r <= len(self) and self[r] < self[smallest]: smallest = r
        if smallest != i:
