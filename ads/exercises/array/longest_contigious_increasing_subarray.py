...
- Example 1:
    ...
    - Explanation: The longest continuous increasing subsequence is [1,3,5]
                   with length 3. Even though [1,3,5,7] is an increasing 
                   subsequence, it is not continuous as elements 5 and 7 are 
...
def len_LCIS(nums: List[int]) -> int:

    res = 0
    anchor = 0
    for i, val in enumerate(nums):
        if i > 0 and nums[i - 1] >= val: anchor = i
